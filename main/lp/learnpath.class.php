    ...
    public function getBuildTree(array $elements, $parentId = null){
        ...
        foreach ($elements as $element){
            ...
            if ($element['id'] == $this->current) {
                $element['parent_current'] = 'in';
                $element['current'] = 'on';
            }

            if (!in_array($element['type'], $dirTypes)) {
                if (isset($classStatus[$element['status']])) {
                    $cssStatus = $classStatus[$element['status']];
                }
                $classStyle = null;
                if ($element['id'] == $this->current) {
                    $classStyle = 'scorm_item_normal '.$classStyle.'scorm_highlight';
                } elseif (!in_array($element['type'], $dirTypes)) {
                    $classStyle = 'scorm_item_normal '.$classStyle;
                }
                $element['class'] = $classStyle.' '.$cssStatus;
                $element['url'] = $this->get_link('http', $element['id'], $elements);
                $element['current_id'] = $currentID;
            }
